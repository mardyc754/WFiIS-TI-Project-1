...
function generateRandomNode(tree){
    ...
    if(min < 1 || min > 999){
        info.innerHTML = "Wartość \"Od\" musi mieć wartość od 1 do 999";
        return;
    }

    if(max < 1 || max > 999){
        info.innerHTML = "Wartość \"Do\" musi mieć wartość od 1 do 999";
        return;
    }
    ...
    tree.existingKeys = tree.filterKeys();
    ...
    else if(tree.existingKeys.length >= 64){
...
function generateNodeByKey(tree){
    const info = document.getElementById("add-node-info");
